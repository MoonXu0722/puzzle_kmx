    ...
    : QWidget(parent)
{
    ...
    std::cout << "map[0][0]:"<< map[0][0].partId << std::endl;
    std::cout << "map[0][1]:"<< map[0][1].partId << std::endl;
    std::cout << "map[0][2]:"<< map[0][2].partId << std::endl;
    std::cout << "map[1][0]:"<< map[1][0].partId << std::endl;
    std::cout << "map[1][1]:"<< map[1][1].partId << std::endl;
    std::cout << "map[1][2]:"<< map[1][2].partId << std::endl;
    std::cout << "map[2][0]:"<< map[2][0].partId << std::endl;
    std::cout << "map[2][1]:"<< map[2][1].partId << std::endl;
    std::cout << "map[2][2]:"<< map[2][2].partId << std::endl;*/
    /*这个函数用来打乱顺序*/
    orderBroken();
    update();
    ...
    QString fileName = tr(":/images/mx.jpg");
   ...
   // btn->setText(tr("选择图片"));
    ...
    startBtn->setGeometry(totalRadius+80,20,120,40);
...
void Widget::paintEvent(QPaintEvent *)
{
    ...
    if(checkGameOver()&&!win){
        map[0][0].partId = 9;
        map[0][0].drawPart(p,img);
        //p.drawImage(totalRadius+50,totalRadius-350,
                    //img->scaled(300,300,Qt::IgnoreAspectRatio),
                    //0,0,300,300);
        win = true;
        QMessageBox box;
        box.setText("康老师你真棒，奖励个灭火器呗！哈哈哈哈");
        box.exec();
    }
}
